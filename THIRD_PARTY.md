...
Historical notebooks are retained for provenance in `notebooks/archive/`. They are not the current implementation and may preserve upstream or course-originated material. Cite Feulner and Clopath (2021) when using the underlying model; full bibliographic records are in `literature/references.bib`.
