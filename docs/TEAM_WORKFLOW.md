...
- **Scientific coordinator:** protects the question, hypotheses, and claim boundary.
...
The pinned status issue receives a short end-of-block comment: completed work, current result, blocker, next owner, and next decision. GitHub records implementation truth; group discussion, abstract workshopping, and TA conversations remain collaborative activities.
...
Only one person edits `notebooks/Motor_RNN_Project.ipynb` at a time. Other contributors should prefer source, tests, literature, or documentation tasks to avoid notebook merge conflicts.
...
2. Open the canonical notebook in Colab using `RUN_MODE="view"`.
