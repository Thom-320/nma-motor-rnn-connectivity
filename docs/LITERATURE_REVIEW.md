...
Five papers that put our question in context, plus one bridge paper. Each entry answers the six questions NMA asks you to answer after reading a paper.

## Why these papers

We're asking whether a denser recurrent network learns to reach better because of its wiring, or just because more of its connections can change. These papers get us to that question:

- **Feulner & Clopath** is the model we use. It fixes density at $p=0.10$ and never varies it.
- **Sussillo & Abbott** is the learning rule (FORCE/RLS).
- **Jaeger & Haas** shows sparse random recurrence can compute well — but with the recurrent weights frozen.
- **Wärnberg & Kumar** links connectivity to which activity patterns a circuit can produce.
- **Gao et al.** is the warning: low dimensionality can come from a simple task, not a simple circuit.
- **Khona et al.** (bridge) shows denser isn't automatically better.

Read one each, then talk them through together before writing anything. Khona motivates the question; it doesn't predict our answer.
